...
class ImportModel:
    ...
    def get_language_model(self):
        ...
        if self.model_type == "LLM":
            ...
            return getattr(self.model_ref, "language_model", getattr(self.model_ref, "text_model", self.model_ref))
        
    def get_prelayers_model(self):
        """ Dynamically pre-Layers """
        if hasattr(self.get_language_model(), "layers"):
            return self.get_language_model()
        else:
            return self.get_language_model().model

        
    def get_config(self):
        """Dynamically retrieves the model's config."""
        language_model = self.get_language_model()
        return language_model.config if language_model else None
