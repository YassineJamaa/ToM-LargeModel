...
class ToMPainLoc(Dataset):
    def __init__(self):
        emotion_file = "dataset/localizer/pain-tom/emotion_pain.txt"
        physical_file = "dataset/localizer/pain-tom/physical_pain.txt"

        with open(emotion_file, 'r', encoding='utf-8') as f:
            self.positive = [f"Story: {line.strip()}" for line in f]

        with open(physical_file, 'r', encoding='utf-8') as f:
            self.negative = [f"Story: {line.strip()}" for line in f]
...
def main():
    ...
    for benchmark_name in benchmark_names:
        ...
        json_file = os.path.join("Result", "FURTHER-ANALYSIS", "absolute", f"TOP_MASK_{localizer}", model_name, f"{benchmark_name}_top_lesion.json")
        ###############################################################
        ...
        for percentage in percentages:
            mask = get_masked_ktop(noabs_loc, percentage)
