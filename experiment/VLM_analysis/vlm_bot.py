...
def parse_arguments():
    ...
    parser.add_argument("--benchmark_names", type=str, nargs='+', choices=["MMStar", "MathVista"], default=["MMStar"], help="List of benchmark names. Choose from: MathVista, MMStar")
...
def save_data_json(benchmark, model_name, dataframe, percentages, predictions):
    json_file = os.path.join("Result_VLM", "VLM_LESION_ToM", model_name, f"{type(benchmark).__name__}_bottom_lesion.json")
...
def assess_benchmark(benchmark, assess, model_name, loc, percentages):
    ...
    for idx, percentage in enumerate(percentages):
        ...
        accuracy = (res_bot["predict_ablate_top"].str.lower() == res_bot["answer_letter"]).mean()
    ...
    save_data_json(benchmark=benchmark,
                   ...
                   predictions=bot_predictions) # Call Function
