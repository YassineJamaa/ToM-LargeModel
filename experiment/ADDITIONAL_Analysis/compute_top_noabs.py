...
def parse_arguments():
    ...
    parser.add_argument("--benchmark_names", type=str, nargs='+', default=["ToMi", "OpenToM", "MATH"], choices=["variant_FanToM", "FanToM", "MATH", "OpenToM", "ToMi"], help="List of benchmark names: MD or ToM")
...
def main():
    ...
    assess = Assessmentv6(import_model=llm,
                        ablation=zeroing,
                        device=device)
    percentages = np.array([0.0, 0.005, 0.01])
    ...
    for benchmark_name in benchmark_names:
        ...
        json_file = os.path.join("Result", "Final-Analysis", "ADDITIONAL-ANALYSIS", "No-absolute", f"TOP_MASK_{localizer}", model_name, f"{benchmark_name}_top_lesion.json")
        ###############################################################
