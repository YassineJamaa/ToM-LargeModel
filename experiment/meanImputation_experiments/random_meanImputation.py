...
def main():
    ...
    assess = Assessmentv4(import_model=llm,
                        ...
                        device=device)
    percentages = np.concatenate(([0, 0.001, 0.002, 0.003], np.linspace(0, 0.025, 26)[::2][2:]))[::2][1:]
    random_predictions = []
    n_random = 50
    ...
    json_file= os.path.join("Result", "PERCENTAGES_ANALYSIS", "Extended_Random_MeanImputation-10-02-2025", model_name, f"{type(benchmark).__name__}_random_meanImputation.json")
    data_dict = {
